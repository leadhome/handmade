...
class ZendX_JQuery_JqGrid
{
     ...
     * @var ZendX_JQuery_JqGrid_Adaptor_Interface
     */
    ...
    /**
     * Instance of ZendX_JQuery_JqGrid_Plugin_Broker
     * 
     * @var unknown_type
     */
    protected $_plugins;
     ...
     * Constructor.
     *
     * @param ZendX_JQuery_JqGrid_Adapter_Interface $adaptor
     * @param array $options
     */
    public function __construct($id, $adapter, array $options = array())
    {
        Zend_Paginator::addAdapterPrefixPath('ZendX_JQuery_JqGrid_Adapter', 'ZendX/JQuery/JqGrid/Adapter');
        
        if ($adapter instanceof ZendX_JQuery_JqGrid_Adapter_Interface) {
            $this->_adapter = $adapter;
        } else {
            throw new ZendX_JQuery_JqGrid_Exception('ZendX_JQuery_JqGrid only accepts instances of the type ' . 'ZendX_JQuery_JqGrid_Adapter_Interface');
        }
        $this->_id = $id;
        ...
        $this->_plugins = new ZendX_JQuery_JqGrid_Plugin_Broker();
        $this->_plugins->setGrid($this);
        
        $this->_plugins->registerPlugin(new ZendX_JQuery_JqGrid_Plugin_Pager());
    }
     ...
     * @return string
     */
    public function setNavOptions(array $options = array())
    {
		foreach ($options as $key => $value)
            $opt .= $key.':'.strval($value).',';
		$opt = substr($opt, 0, strlen($opt)-1);
		$pagerName = $this->getOption('pager');
		$js = sprintf('$(document).ready(function() { %s("#%s").navGrid("#%s",{%s}) });',
				ZendX_JQuery_View_Helper_JQuery::getJQueryHandler(),
				$this->_id,
				$pagerName,
				$opt);
		$scripts = $this->getView()->inlineScript();
		$scripts->appendScript($js);
		return true;
    }
     ...
     * 
     * @param ZendX_JQuery_JqGrid_Column $column
     * @return ZendX_JQuery_JqGrid
     */
    ...
    public function render(Zend_View_Interface $view = null)
    {
        ...
        $view = $this->getView();
        $view->addHelperPath("ZendX/JQuery/JqGrid/View/Helper", "ZendX_JQuery_JqGrid_View_Helper");
    ...
    protected function _createGridData(Zend_Controller_Request_Abstract $request)
    {
        ...
        if ($request->getParam('_search') == 'true') {
            ...
            $this->_paginator->getAdapter()->filter($filter['items'], $filter['options']);
        }
        ...
        foreach ($rows as $k => $row) {
            ...
            foreach ($this->_columns as $column) {
                if ($f = $column->getOption('build'))
                    $value = $f($row, $k);
                else
                    $value = $column->cellValue($row);
                array_push($grid->rows[$k]['cell'], $value);
            }
        }
        return $grid;
    }
    ...
    private function _getFilterParams($request)
    {
        ...
        if ($request->getParam('filters')) {
            $filter = Zend_Json::decode(stripslashes($request->getParam('filters')));
            if (count($filter['rules']) > 0) {
                foreach ($filter['rules'] as $rule) {
                    $f = $this->_columns[$rule['field']]->getOption('unbuild');
                    $filters['items'][] = array(
                        'field' => ($f != null) ? $f : $rule['field'],
                        'value' => $rule['data'],
                        'expression' => $this->_expression[$rule['op']]);
                }
