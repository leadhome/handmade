<?php
/**
 * Исправление "клавиатурных" опечаток в тексте подобно программе Punto Switcher, Keyboard Ninja, KeySwitcher.
 *
 * НАЗНАЧЕНИЕ
 *   Корректировка поисковых запросов, существующих и новых текстов, публикуемых посетителями на веб-сайтах.
 *
 * ВОЗМОЖНОСТИ
 *   В тексте исправляет ошибочно набранные буквы в словах,
 *   которые выглядят одинаково или набраны в другой раскладке клавиатуры.
 *   Незаметные латинские буквы среди русских исправляются в русские и наоборот.
 *   Двухсторонняя поддержка для языков: русский <--> английский.
 ...
 *
 * ЗАМЕЧАНИЕ
 *   В редких случаях возможны ошибки в коррекции слов.
 ...
 *     3. Иначе скорректировать запрос через Text_WordsFakeFix;
 ...
 *
 * Punto Switcher
 *   Рекомендую пользоваться программой Punto Switcher <http://punto.yandex.ru/>
 *   Исправление отдельных букв из другой раскладки клавиатуры Punto Switcher 3.0
 *   пока делать не умеет, но, будем надеяться, что разработчики сделают улучшение в будущих версиях.
 *
 * @created  2008-08-30
 * @license  http://creativecommons.org/licenses/by-nc-sa/3.0/
 * @author   Nasibullin Rinat: http://orangetie.ru/, http://rin-nas.moikrug.ru/
 * @charset  UTF-8 / don't change!
 * @version  1.4.0
 */
class pEngine_Text_Fix
{
	/**
	 * Константа для исправляния ошибочно набранных букв в словах,
	 * которые выглядят одинаково в разных раскладках клавиатуры.
	 * Алгоритм работает достаточно надёжно.
	 */
	const SIMILAR_CHARS = 1;

	/**
	 * Константа для исправляния ошибочно набранных слов в другой раскладке клавиатуры.
	 * Алгоритм может иногда ошибаться.
	 */
	...
	*/
	 ...
	 *                                 При использовании self::KEYBOARD_LAYOUT время работы
	...
	public function parse($s, $mode = self::SIMILAR_CHARS, array &$words = null)
	{
		...
		if ($mode < self::SIMILAR_CHARS || $mode > (self::SIMILAR_CHARS | self::KEYBOARD_LAYOUT))
		{
		...
		$s = pEngine_Text_UTF8::diactrical_remove($s, $additional_chars, $is_can_restored = true, $restore_table);

		$this->words = array();
		$s = $this->_parse1($s);
		$s = $this->_parse2($s);
		$s = pEngine_Text_UTF8::diactrical_restore($s, $restore_table);
		$words = $this->words;
		return $s;
	}
	...
	private function _parse1($s)
	{
		...
		return preg_replace_callback('/(?> (' . $this->en . ')  #1 латинские буквы
                                         |  ' . $this->sc . '   #символы, которые м.б. набраны по ошибке в английской раскладке клавиатуры вместо русских букв
	...
	private function _word(array &$a)
	{
		...
		$c = count($suggestions);
		if ($c === 0) $s = $word;
		else $s = $this->_detect($word, $suggestions);
		if ($s !== $word) $this->words[$word] = $s;
		return $s;
	}
	...
	private function _detect($word, array $suggestions)
	{
		if (0) #DEBUG
		{
			//$suggestions['?'] = $word;
	...
	#анализ на основе N-грам русского и английского языка
	private function _bigram_exists($word, $lang)
	{
		$word = ($lang === 'en') ? strtolower($word) : pEngine_Text_UTF8::lowercase($word);
		...
		#шаг 3.
		$length = pEngine_Text_UTF8::strlen($word);
		for ($pos = 0, $limit = $length - 1; $pos < $limit; $pos++)
		{
			...
			*/
			$ss = pEngine_Text_UTF8::substr($word, $pos, 2);
